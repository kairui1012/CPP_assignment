...
class HashTable {
    ...
    public:
        ...
        void insert(const string& word) {
            unsigned long bucketIndex = hashFunction(word);
            WordNode* current = buckets[bucketIndex];
            ...
            // 插入新节点到头部
            buckets[bucketIndex] = new WordNode{word, 1, buckets[bucketIndex]};
        }
    
        WordNode* mergeAllBuckets() {
            WordNode* mergedHead = nullptr;
            for (int i = 0; i < HASH_TABLE_SIZE; ++i) {
                WordNode* current = buckets[i];
                while (current) {
                    WordNode* next = current->next;
                    current->next = mergedHead;
                    mergedHead = current;
                    current = next;
                }
                buckets[i] = nullptr; // 清空桶
            }
            return mergedHead;
        }
    };
...
WordNode* calculateWords(NewsNode* newsHead) { 
    ...
    HashTable wordTable; // 使用哈希表代替原链表
    int lines = 0;
    ...
    while (newsHead) {
        ...
        for (; it != end; ++it) {
            if (!it->str().empty()) {
                wordTable.insert(it->str()); // 插入到哈希表
            }
        }
    ...
    return wordTable.mergeAllBuckets(); // 合并所有桶到链表
}
int main()
{
